...
from .load_NHR import load_NHR_data,NHR_Dataset
...
def load_data(args):
    ...
    if args.dataset_type == 'llff':
        ...
        images, poses, render_poses, hwf, K, i_split = load_NHR_data(args.datadir,args.frame)
...
def load_data_frame(args,frame_id, pre_no = 8 ,  only_current = False, scale = 1.0):
    '''if train_mode = individual, onlu current - True'''
    K, depths = None, None

    if pre_no <0:
        pre_no = frame_id

    previous_frame_ids = np.random.permutation(frame_id)
    N = pre_no
    if frame_id< N - 1:
        N = frame_id 
    if N<=0:
        ...
        previous_frame_ids = previous_frame_ids[:N].tolist()

    if only_current:
        previous_frame_ids  = []

    if args.dataset_type == 'NHR':
        dataset = NHR_Dataset(args.datadir,200)
        images, poses, render_poses, hwf, K, i_split, frame_ids = dataset.load_data(frame_id, previous_frame_ids, scale = scale)
        ...
        i_replay=None
        i_current=np.arange(0,images.shape[0])
        frame_ids=torch.ones(poses.shape[0]).long()*int(frame_id)
        print(frame_ids)
    ...
    data_dict = dict(
        hwf=hwf, HW=HW, Ks=Ks, near=near, far=far,
        i_train=i_train, i_val=i_val, i_test=i_test, i_replay=i_replay, i_current=i_current,
        poses=poses, render_poses=render_poses,
        images=images, depths=depths,
        irregular_shape=irregular_shape, frame_ids = frame_ids
    )
    return data_dict
