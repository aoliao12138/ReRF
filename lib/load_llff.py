...
def _load_data(basedir, factor=None, width=None, height=None, load_imgs=True, load_depths=False,frame_id=-1):

    poses_arr = np.load(os.path.join(basedir, f'{frame_id}/poses_bounds.npy'))
    ...
    img0 = [os.path.join(basedir, f'{frame_id}/images', f) for f in sorted(os.listdir(os.path.join(basedir, f'{frame_id}/images'))) \
    ...
    if height is not None and width is not None:
        _minify(basedir+f'/{frame_id}', resolutions=[[height, width]])
        sfx = '_{}x{}'.format(width, height)
    elif factor is not None and factor != 1:
        sfx = '_{}'.format(factor)
        _minify(basedir+f'/{frame_id}', factors=[factor])
        factor = factor
    ...
    imgdir = os.path.join(basedir+f'/{frame_id}', 'images' + sfx)
    if not os.path.exists(imgdir):
