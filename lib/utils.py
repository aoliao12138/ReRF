...
def create_optimizer_or_freeze_model_frame(model, cfg_train, global_step, fix_rgb = False,
                                           deform_stage=False,res_stage=False ):
    ...
    for k in cfg_train.keys():
        ...
        for id in model.activated_dvgos:
            sub_model = model.dvgos[id]
            if not hasattr(sub_model, k):
                continue

            param = getattr(sub_model, k)
            if param is None:
                print(f'create_optimizer_or_freeze_model: param {k} not exist')
                continue
            ...
            lr = getattr(cfg_train, f'lrate_{k}') * decay_factor
            if deform_stage:
                if k=='density' or k=='k0' or k=='rgbnet':
                    lr=0
            if res_stage:
                if k== 'deformation_field':
                    lr=0

            if lr > 0:
                print(f'create_optimizer_or_freeze_model: param {k} lr {lr}')
                if isinstance(param, nn.Module):
                    if k=='k0' and hasattr(param,'former_k0'):
                        #param.former_k0.requires_grad = False
                        param=param.k0
                        print('optimize ', 'k0.k0')
                    else:
                        print('optimize ',k,'....')
                        param = param.parameters()

                param_group.append({'params': param, 'lr': lr, 'skip_zero_grad': (k in cfg_train.skip_zero_grad_fields), 'frame_id': id})
            else:
                print(f'create_optimizer_or_freeze_model: param {k} freeze')
                param.requires_grad = False
...
def create_optimizer_freeze_volume_frame(model, cfg_train, global_step,fix_rgb = True):
    decay_steps = cfg_train.lrate_decay * 1000
    decay_factor = 0.1 ** (global_step/decay_steps)
    ...
    k='refinenet'
    param = model.refinenet
    for i in range(1):
        if param is None:
            print(f'create_optimizer_or_freeze_model: param {k} not exist')
            continue
        if fix_rgb:
            print(f'Fix refinenet mode.')
            continue 

        lr = 6e-4 * decay_factor
        if lr > 0:
            print(f'create_optimizer_or_freeze_model: param {k} lr {lr}')
            if isinstance(param, nn.Module):
                param = param.parameters()
